...
document.addEventListener('DOMContentLoaded', function() {
    ...
    normalizeVendas();
    // Normalizar compras: migrar valorUnitario para valorCusto/valorRevenda, garantir ids
    normalizeCompras();
...
function normalizeCompras() {
    let changed = false;
    compras = compras || [];
    compras.forEach(c => {
        // garantir id
        if (!c._id) { c._id = 'cmp_' + Date.now() + '_' + Math.floor(Math.random()*1000000); changed = true; }
        // migrar valorUnitario para valorCusto/valorRevenda caso não existam
        if ((c.valorCusto === undefined || c.valorCusto === null) && typeof c.valorUnitario !== 'undefined') {
            c.valorCusto = Number(c.valorUnitario);
            changed = true;
        }
        if ((c.valorRevenda === undefined || c.valorRevenda === null) && typeof c.valorUnitario !== 'undefined') {
            c.valorRevenda = Number(c.valorUnitario);
            changed = true;
        }
    });
    if (changed) {
...
function atualizarPrecoCompra() {
    const el = document.getElementById('compraProduto');
    if (!el) return;
    const nome = (el.value || '').toString().trim();
    if (!nome) return;
    const item = (estoque || []).find(p => (p.nome || '').toString().trim().toLowerCase() === nome.toLowerCase());
    if (item) {
        if (document.getElementById('compraValorRevenda')) document.getElementById('compraValorRevenda').value = Number(item.preco || 0).toFixed(2);
        // Keep custo separate (we try to fill from last purchase below), custo does not change stock price
        calcularValorTotalCompra();
        return;
    }
    // fallback: find last purchase price
    const comp = compras.slice().reverse().find(c => c.produto && c.produto.toLowerCase() === nome.toLowerCase());
    if (comp) {
        if (document.getElementById('compraValorCusto')) document.getElementById('compraValorCusto').value = (comp.valorCusto || comp.valorUnitario || 0).toFixed(2);
        if (document.getElementById('compraValorRevenda')) document.getElementById('compraValorRevenda').value = (comp.valorRevenda || comp.valorUnitario || 0).toFixed(2);
        if (document.getElementById('compraQtd')) document.getElementById('compraQtd').value = comp.quantidade || 1;
        calcularValorTotalCompra();
    }
}
...
function renderizarCompras() {
    ...
    dados.forEach((compra, idx) => {
        // prefer _id when possible
        let index = -1;
        if (compra._id) index = compras.findIndex(c => c && c._id === compra._id);
        if (index === -1) {
            index = compras.findIndex(c => c && c.data === compra.data && c.produto === compra.produto && String(c.valorTotal) === String(compra.valorTotal));
        }
        const tr = document.createElement('tr');
        tr.innerHTML = `
            <td>${compra.data || ''}</td>
            ...
            <td>${formatBRL(compra.valorCusto || compra.valorUnitario || 0)}</td>
            <td>${formatBRL(compra.valorRevenda || compra.valorUnitario || 0)}</td>
            <td>${formatBRL(compra.valorTotal || 0)}</td>
            <td>${compra.fornecedor || '-'}</td>
            <td>
                <button onclick="editarCompra(${index})" title="Editar">✏️</button>
...
function renderizarFinanceiro() {
    ...
    if (resumo) {
        ...
        const totalCompras = dados.filter(i=>i.tipo==='Compra').reduce((s,i)=>s+Number(i.valor||0),0);
        const saldo = totalReceitas - (totalDespesas + totalCompras);
        resumo.textContent = `Total Receitas: ${formatBRL(totalReceitas)}    Total Despesas: ${formatBRL(totalDespesas)}    Total Compras: ${formatBRL(totalCompras)}    Saldo: ${formatBRL(saldo)}`;
    }
}
...
function limparFormFinanceiro() {
    ...
    if (tipoEl) {
        // remover opção temporária 'Compra' quando estiver limpando o formulário
        const opt = tipoEl.querySelector('option[value="Compra"]');
        if (opt) tipoEl.removeChild(opt);
        tipoEl.disabled = false;
    }
    const tipoNote = document.getElementById('finTipoNote');
    if (tipoNote) tipoNote.textContent = '';
    window._editingDespesaOriginalTipo = null;
}
...
function editarDespesa(id) {
    ...
    const tipoEl = document.getElementById('finTipo');
    window._editingDespesaOriginalTipo = item.tipo || null;
    if (item.tipo === 'Compra') {
        // Mostrar nota e desabilitar a seleção para evitar alteração acidental
        if (tipoEl) {
            // adicionar opção temporária para exibir 'Compra (importada)' sem permitir seleção em novo registro
            let opt = tipoEl.querySelector('option[value="Compra"]');
            if (!opt) {
                opt = document.createElement('option');
                opt.value = 'Compra';
                opt.text = 'Compra (importada)';
                opt.disabled = true;
        ...
        const tipoNote = document.getElementById('finTipoNote');
        if (tipoNote) tipoNote.textContent = 'Compra (importada) — tipo não editável aqui';
    } else {
    ...
    form.onsubmit = function(e){
        e.preventDefault();
        const data = document.getElementById('finData').value;
        let tipo = document.getElementById('finTipo').value;
        // Se estivermos editando um lançamento importado como Compra e o select está desabilitado,
        // preservamos o tipo original ('Compra') em vez de sobrescrever com o valor do select.
        if (window._editingDespesaOriginalTipo === 'Compra') {
            tipo = 'Compra';
        }
        const categoria = document.getElementById('finCategoria').value;
        const valor = parseFloat(document.getElementById('finValor').value) || 0;
...
// importarDespesasParaFinanceiro removed: purchases should not be auto-imported as despesas.

function importarComprasParaFinanceiro() {
    // importa compras como lançamentos do tipo 'Compra' (evita duplicar através da propriedade compraId)
    const comp = JSON.parse(localStorage.getItem('compras')) || compras || [];
    let added = 0;
    comp.forEach((c, idx) => {
        // verificar se já existe lançamento referenciando essa compra (usar _id quando disponível)
        const compraRefId = c._id || ('cmp_' + idx);
        const exists = despesas.some(d => d.compraId === compraRefId || d.compraRef === (c.nf || c.produto));
        if (!exists) {
            const id = 'dsp_' + Date.now() + '_' + idx;
            despesas.push({ _id: id, data: c.data || '', tipo: 'Compra', categoria: 'Compra', valor: Number(c.valorTotal || 0), fornecedor: c.fornecedor || '', descricao: 'Compra: ' + (c.produto||''), compraId: compraRefId });
            added++;
        }
    });
    if (added > 0) {
...
function exportFinanceiroPDF() {
    ...
    if (doc.autoTable) {
        ...
        if (pageHeight - M - finalY >= footerH) {
            doc.text(`Total Receitas: ${formatBRL(totalReceitas)}    Total Despesas: ${formatBRL(totalDespesas)}    Total Compras: ${formatBRL(totalCompras)}    Saldo: ${formatBRL(saldo)}`, M, finalY + 6);
        } else {
            doc.addPage();
            gerarCabecalhoPDF(doc, 'Relatório Financeiro', dataHora, 'admin', M);
            doc.text(`Total Receitas: ${formatBRL(totalReceitas)}    Total Despesas: ${formatBRL(totalDespesas)}    Total Compras: ${formatBRL(totalCompras)}    Saldo: ${formatBRL(saldo)}`, M, M + headerH + 6);
        }
    } else {
        ...
        doc.setFontSize(11); doc.text(`Total Receitas: ${formatBRL(totalReceitas)}    Total Despesas: ${formatBRL(totalDespesas)}    Total Compras: ${formatBRL(totalCompras)}    Saldo: ${formatBRL(saldo)}`, M, y+8);
    }
...
function exportFinanceiroExcel() {
    ...
    const somaCompras = dados.filter(i=>i.tipo==='Compra').reduce((s,i)=>s+Number(i.valor||0),0);
    rows.push([]);
    rows.push(['Totais','','','Receitas', somaReceitas, '']);
    rows.push(['Totais','','','Despesas', somaDespesas, '']);
    rows.push(['Totais','','','Compras', somaCompras, '']);
    rows.push(['Saldo','','','', somaReceitas - (somaDespesas + somaCompras), '']);
...
function calcularValorTotalCompra() {
    ...
    const valorCusto = parseFloat((document.getElementById('compraValorCusto') || {}).value) || 0;
    document.getElementById('compraValorTotal').value = (quantidade * valorCusto).toFixed(2);
}
...
function registrarCompra(event) {
    ...
    const compra = {
        ...
        valorCusto: parseFloat((document.getElementById('compraValorCusto') || {}).value) || parseFloat((document.getElementById('compraValorUnitario') || {}).value) || 0,
        valorRevenda: parseFloat((document.getElementById('compraValorRevenda') || {}).value) || parseFloat((document.getElementById('compraValorUnitario') || {}).value) || 0,
    ...
    // Se estivermos editando, atualizar o registro e ajustar estoque
    if (compraEditando !== null && compras[compraEditando]) {
        const antigo = compras[compraEditando];
        // Ajustar estoque: se produto igual, aplicar diferença; se diferente, remover do antigo e acrescentar ao novo
        try {
            if ((antigo.produto || '').toLowerCase() === (compra.produto || '').toLowerCase()) {
                const prod = estoque.find(item => item.nome && item.nome.toLowerCase() === compra.produto.toLowerCase());
                if (prod) {
                    prod.quantidade = (prod.quantidade || 0) + (compra.quantidade - (antigo.quantidade || 0));
                    // Estoque.preco is used as the resale price in vendas; prefer valorRevenda, fallback to valorCusto
                    if (compra.valorRevenda > 0) prod.preco = compra.valorRevenda;
                    else if (compra.valorCusto > 0) prod.preco = compra.valorCusto;
                    if (compra.fornecedor) prod.fornecedor = compra.fornecedor;
                    if (compra.tipo) prod.tipo = compra.tipo;
                }
            } else {
                const prodAnt = estoque.find(item => item.nome && item.nome.toLowerCase() === (antigo.produto || '').toLowerCase());
                if (prodAnt) prodAnt.quantidade = Math.max(0, (prodAnt.quantidade || 0) - (antigo.quantidade || 0));
                const prodNovo = estoque.find(item => item.nome && item.nome.toLowerCase() === (compra.produto || '').toLowerCase());
                if (prodNovo) {
                    prodNovo.quantidade = (prodNovo.quantidade || 0) + compra.quantidade;
                    if (compra.valorRevenda > 0) prodNovo.preco = compra.valorRevenda;
                    else if (compra.valorCusto > 0) prodNovo.preco = compra.valorCusto;
                    if (compra.fornecedor) prodNovo.fornecedor = compra.fornecedor;
                    if (compra.tipo) prodNovo.tipo = compra.tipo;
                } else {
                    estoque.push({ nome: compra.produto, tipo: compra.tipo || 'Insumo', quantidade: compra.quantidade, unidade: 'un', preco: compra.valorRevenda > 0 ? compra.valorRevenda : compra.valorCusto, fornecedor: compra.fornecedor });
                }
            }
        } catch (e) { console.warn('Erro ao ajustar estoque na edição compra', e); }

        // manter id do registro
        compra._id = antigo._id || ('cmp_' + Date.now() + '_' + Math.floor(Math.random()*10000));
        compras[compraEditando] = compra;
        compraEditando = null;
        try { document.getElementById('btnRegistrarCompra').textContent = '✅ Registrar Compra'; } catch(e){}
        try { document.getElementById('btnCancelarEdicaoCompra').style.display = 'none'; } catch(e){}
        localStorage.setItem('estoque', JSON.stringify(estoque));
        localStorage.setItem('compras', JSON.stringify(compras));
    } else {
        ...
        if (produtoExistente) {
            ...
            if (compra.valorRevenda > 0) {
                produtoExistente.preco = compra.valorRevenda;
            } else if (compra.valorCusto > 0) {
                produtoExistente.preco = compra.valorCusto;
            }
            ...
            estoque.push({
                ...
                unidade: 'un',
                preco: compra.valorRevenda > 0 ? compra.valorRevenda : compra.valorCusto,
        ...
        // Registrar compra
        compra._id = compra._id || ('cmp_' + Date.now() + '_' + Math.floor(Math.random()*10000));
        compras.push(compra);
        localStorage.setItem('compras', JSON.stringify(compras));
    }
...
function limparFormCompra() {
    ...
    if (document.getElementById('compraValorCusto')) document.getElementById('compraValorCusto').value = '';
    if (document.getElementById('compraValorRevenda')) document.getElementById('compraValorRevenda').value = '';
    document.getElementById('compraData').value = new Date().toISOString().split('T')[0];
    const erroDiv = document.getElementById('erro-compraFornecedor');
    if (erroDiv) erroDiv.classList.add('hidden');
    // Reset editing UI when clearing form
    compraEditando = null;
    try { document.getElementById('btnRegistrarCompra').textContent = '✅ Registrar Compra'; } catch(e){}
    try { document.getElementById('btnCancelarEdicaoCompra').style.display = 'none'; } catch(e){}
}
...
function editarCompra(index) {
    if (typeof index !== 'number' || index < 0 || index >= compras.length) return;
    const compra = compras[index];
    if (!compra) return;

    document.getElementById('compraProduto').value = compra.produto || '';
    if (document.getElementById('compraTipo')) document.getElementById('compraTipo').value = compra.tipo || 'Insumo';
    document.getElementById('compraQtd').value = compra.quantidade || 0;
    if (document.getElementById('compraValorCusto')) document.getElementById('compraValorCusto').value = (compra.valorCusto || compra.valorUnitario) || '';
    if (document.getElementById('compraValorRevenda')) document.getElementById('compraValorRevenda').value = (compra.valorRevenda || compra.valorUnitario) || '';
    document.getElementById('compraValorTotal').value = compra.valorTotal || '';
    document.getElementById('compraFornecedor').value = compra.fornecedor || '';
    if (document.getElementById('compraNF')) document.getElementById('compraNF').value = compra.nf || '';
    if (document.getElementById('compraData')) document.getElementById('compraData').value = compra.data || new Date().toISOString().split('T')[0];

    compraEditando = index;
    try { document.getElementById('btnRegistrarCompra').textContent = '💾 Atualizar Compra'; } catch(e){}
    try { document.getElementById('btnCancelarEdicaoCompra').style.display = 'inline-block'; } catch(e){}
    try { routeMenu('compras'); } catch(e){}
}
...
function excluirCompra(index) {
    if (confirm('Tem certeza que deseja excluir esta compra?')) {
        // Ajustar estoque: reduzir a quantidade do produto correspondente
        const comp = compras[index];
        if (comp && comp.produto) {
            const prod = estoque.find(p => p.nome && p.nome.toLowerCase() === comp.produto.toLowerCase());
            if (prod) {
                prod.quantidade = Math.max(0, (prod.quantidade || 0) - (comp.quantidade || 0));
            }
        }
    ...
    nomes.forEach(nome => {
        ...
        div.addEventListener('click', () => {
            ...
            const comprasDoProduto = compras.filter(c => c.produto && c.produto.toLowerCase() === nome.toLowerCase());
                ...
                document.getElementById('prodPreco').value = (ultimaCompra.valorRevenda || ultimaCompra.valorUnitario || ultimaCompra.valorCusto) ? parseFloat(ultimaCompra.valorRevenda || ultimaCompra.valorUnitario || ultimaCompra.valorCusto).toFixed(2) : '';
...
function verificarEnterProduto(event) {
    if (event.key === 'Enter') {
        ...
        if (compra) {
            ...
            document.getElementById('prodPreco').value = (compra.valorRevenda || compra.valorUnitario || compra.valorCusto) ? parseFloat(compra.valorRevenda || compra.valorUnitario || compra.valorCusto).toFixed(2) : '';
...
function gerarRelatorio() {
    ...
    switch (tipo) {
        ...
        case 'compras':
            html += `<div class="table-container"><table><thead><tr><th>Data</th><th>Produto</th><th>Tipo</th><th>NF</th><th>Quantidade</th><th>Unidade</th><th>Valor de Custo (R$)</th><th>Valor Revenda (R$)</th><th>Valor Total (R$)</th><th>Fornecedor</th></tr></thead><tbody>`;
            dados.forEach(d => {
                html += `<tr><td>${d.data || ''}</td><td>${d.produto || ''}</td><td>${d.tipo || ''}</td><td>${d.nf || ''}</td><td>${d.quantidade || 0}</td><td>${d.unidade || ''}</td><td>${formatBRL(d.valorCusto || d.valorUnitario || 0)}</td><td>${formatBRL(d.valorRevenda || d.valorUnitario || 0)}</td><td>${formatBRL(d.valorTotal || 0)}</td><td>${d.fornecedor || ''}</td></tr>`;
            });
            html += `</tbody></table></div>`;
    ...
    switch(tipo) {
                        ...
                        <tbody>
                            ${comprasFiltradas.map(c => `
                                <tr>
                                    ...
                                    <td>${c.produto}</td>
                                    <td>${c.quantidade}</td>
                                    <td>${formatBRL(c.valorCusto || c.valorUnitario || 0)}</td>
                                    <td>${formatBRL(c.valorRevenda || c.valorUnitario || 0)}</td>
        ...
        case 'inadimplentes': {
            ...
            for (const clienteNome of Object.keys(agrup)) {
                ...
                doc.setFontSize(12);
                doc.text(sanitizeForExport(clienteNome), 15, y); y += 8;
                ...
                y += 6;
                doc.setFontSize(12);
                doc.text(`Total devedor do ${sanitizeForExport(clienteNome)}: ${formatBRL(subtotal)}`, 15, y);
                y += 10;
            }
            y += 6;
            doc.setFontSize(14);
...
function exportRelatorioPDF() {
    ...
    function drawTable(columns, rows, startY) {
        ...
        if (doc.autoTable) {
            ...
            let yy = startY;
            // Cabeçalho
            const headerText = sanitizeForExport(columns.join(' | '));
            doc.setFontSize(10);
            doc.text(headerText, M, yy);
            yy += 8;
            rows.forEach(r => {
                const line = r.map(c => (c === null || c === undefined) ? '' : sanitizeForExport(String(c))).join(' | ');
                doc.text(line, M, yy);
                yy += 7;
    ...
    switch(tipo) {
        case 'vendas': {
            ...
            Object.keys(formas).forEach(f => {
                ...
                doc.text(`${sanitizeForExport(f)}: ${formatBRL(val)} (${pct.toFixed(1)}%)`, 20, y);
                y += 8;
            });
            break;
        }
        ...
        case 'compras': {
            ...
            const colsC = ['Data', 'Produto', 'NF', 'Quantidade', 'Unid', 'Valor de Custo (R$)', 'Valor Revenda (R$)', 'Valor Total (R$)', 'Fornecedor'];
            const rowsC = comprasFiltradas.map(c => [c.data || '', c.produto || '', c.nf || '', c.quantidade || 0, c.unidade || '', formatBRL(Number(c.valorCusto || c.valorUnitario || 0)), formatBRL(Number(c.valorRevenda || c.valorUnitario || 0)), formatBRL(Number(c.valorTotal || 0)), c.fornecedor || '-']);
        ...
        case 'inadimplentes': {
            ...
            for (const clienteNome of Object.keys(agrup)) {
                ...
                let yy = 70;
                doc.setFontSize(12);
                doc.text(sanitizeForExport(clienteNome), 15, yy); yy += 8;
                const colsInad = ['Data', 'Horário', 'Produto', 'Quantidade', 'Valor (R$)', 'Dias Atraso'];
                const rowsInad = agrup[clienteNome].map(item => {
                    const dias = calcularDiasAtraso(item.data);
                    return [item.data || '', item.hora || '', sanitizeForExport(item.produto || ''), Number(item.quantidade || 0), formatBRL(Number(item.valor || 0)), dias];
                ...
                yy += 6;
                doc.setFontSize(12);
                doc.text(`Total devedor do ${sanitizeForExport(clienteNome)}: ${formatBRL(subtotal)}`, 15, yy);
            }
    ...
    switch(tipo) {
        ...
        case 'compras':
            dados = obterDadosFiltrados().map(compra => [
                ...
                compra.nf || '',
                compra.quantidade,
                Number(compra.valorCusto || compra.valorUnitario || 0),
                Number(compra.valorRevenda || compra.valorUnitario || 0),
                Number(compra.valorTotal || 0),
                compra.fornecedor || '-'
            ]);
            
            // Adicionar cabeçalho da tabela (inclui NF)
            headerData.push(['Data', 'Produto', 'NF', 'Quantidade', 'Valor de Custo (R$)', 'Valor Revenda (R$)', 'Valor Total (R$)', 'Fornecedor']);
            break;
...
function gerarCabecalhoPDF(doc, titulo, dataHora, usuario, margin = 10) {
    ...
    doc.text(sanitizeForExport(titulo), margin + contentWidth / 2, margin + (bannerH / 2) + 3, { align: 'center' });
    ...
    doc.text(`Usuário: ${sanitizeForExport(usuario)}`, margin + contentWidth, margin + bannerH + 12, { align: 'right' });
...
function exportTableAsPDF(tableId, filename) {
    ...
    const headers = Array.from(table.querySelectorAll('thead th')).map(th => sanitizeForExport(th.textContent.trim()));
    const bodyRows = Array.from(table.querySelectorAll('tbody tr')).map(tr => {
        return Array.from(tr.querySelectorAll('td')).map(td => sanitizeForExport(td.textContent.trim()));
    });
...
function backupDatabase() {
    ...
    const filename = `comercilar_backup_${new Date().toISOString().slice(0,19).replace(/[:T]/g,'-')}.json`;
...
async function uploadBackupToCloud(provider) {
    ...
    try {
        if (provider === 'gdrive') {
            ...
            const boundary = '-------comercilar-boundary-' + Date.now();
            const reader = new FileReader();
